...
namespace CreatorPlanets
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            planetsListing.Add(new Properties { name = "Tatooine", rotation_period = "23", orbital_period = "304", diameter = "10465", climate = "arid", gravity = "1 standard", terrain = "desert", surface_water = "1", population = "200000", residents = 10, films = 5, url = "http://swapi.dev/api/planets/1/", created = TodayisToday, edited = now, id = 1 });
            planetsListing.Add(new Properties { name = "Alderaan", rotation_period = "24", orbital_period = "364", diameter = "12500", climate = "temperate", gravity = "1 standard", terrain = "grasslands, mountains", surface_water = "40", population = "2000000000", residents = 3, films = 2, url = "http://swapi.dev/api/planets/2/", created = TodayisToday, edited = now, id = 2 });
            planetsListing.Add(new Properties { name = "Yavin IV", rotation_period = "24", orbital_period = "4818", diameter = "10200", climate = "temperate", gravity = "1 standard", terrain = "jungle, rainforests", surface_water = "8", population = "1000", residents = 0, films = 1, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 3 });
            planetsListing.Add(new Properties { name = "Hoth", rotation_period = "23", orbital_period = "341", diameter = "7200", climate = "frozen", gravity = "1.1 standard", terrain = "tundra, ice caves, mountain ranges", surface_water = "100", population = "unknown", residents = 0, films = 1, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 4 });
            planetsListing.Add(new Properties { name = "Dagobah", rotation_period = "23", orbital_period = "549", diameter = "8900", climate = "murky", gravity = "N/A", terrain = "swamp, jungles", surface_water = "8", population = "unknown", residents = 0, films = 3, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 5 });
            planetsListing.Add(new Properties { name = "Bespin", rotation_period = "12", orbital_period = "5110", diameter = "118000", climate = "temperate", gravity = "1.5 (surface), 1 standard (Cloud City)", terrain = "gas giant", surface_water = "0", population = "6000000", residents = 1, films = 1, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 6 });
            planetsListing.Add(new Properties { name = "Endor", rotation_period = "18", orbital_period = "402", diameter = "4900", climate = "temperate", gravity = "0.85 standard", terrain = "forests, mountains, lakes", surface_water = "8", population = "30000000", residents = 1, films = 1, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 7 });
            planetsListing.Add(new Properties { name = "Naboo", rotation_period = "26", orbital_period = "312", diameter = "12120", climate = "temperate", gravity = "1 standard", terrain = "grassy hills, swamps, forests, mountains", surface_water = "12", population = "4500000000", residents = 11, films = 4, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 8 });
            planetsListing.Add(new Properties { name = "Coruscant", rotation_period = "24", orbital_period = "368", diameter = "12240", climate = "temperate", gravity = "1 standard", terrain = "cityscape, mountains", surface_water = "unknown", population = "1000000000000", residents = 3, films = 4, url = "http://swapi.dev/api/planets/3/", created = TodayisToday, edited = now, id = 9 });

            Console.WriteLine("<----------------------------------------------------------------------------------------->");
            Console.WriteLine("<-----------------------------Universe Creator V.1.0-------------------------------------->");
            Console.WriteLine("<------------------------------STAR WARS Corporation-------------------------------------->");
            Console.WriteLine("<-----------------------------The Force May with You------------------------------------->");
            Console.WriteLine("<----------------------------------------------------------------------------------------->");
            ...
            switch (userSelection)
            {
                ...
                case 3 when (userSelection == 3):
                    ...
                    if (planetEdit >= 1)
                    {
                        ...
                        if (rotationOneUpdated == 1)
                        {
                            ...
                            var found = planetsListing.FirstOrDefault(p => p.id == planetEdit);
                            if (found != null) { found.rotation_period = rotationInput; found.edited = now; }
                        }
                        else
                        {
                        ...
                        if (orbitOneUpdated == 1)
                        {
                            ...
                            var found = planetsListing.FirstOrDefault(p => p.id == planetEdit);
                            if (found != null) { found.orbital_period = orbitInput; found.edited = now; }
                        }
                        else
                        {
                            Console.WriteLine("Se mantiene el periodo orbital.");
                        }
                        Console.WriteLine(editQMark + el + diameter + "?");
                        Console.WriteLine(editConfirmation);
                        ...
                        if (diameterOneUpdated == 1)
                        {
                            ...
                            var found = planetsListing.FirstOrDefault(p => p.id == planetEdit);
                            if (found != null) { found.diameter = diameterInput; found.edited = now; }
                        }
                        else
                        {
                        ...
                        int residentsUpdate = int.Parse(residentsOne);
                ...
                case 4 when (userSelection == 4):
                    ...
                    var foundPlanet = planetsListing.FirstOrDefault(p => p.id == planetaARemover);
                ...
                case 5 when (userSelection == 5):
                    ...
                    {
                        name = nameNewPlanet,
                        rotation_period = rotationNewPlanet,
                        orbital_period = orbitalNewPlanet,
                        diameter = dimaeterNewPlanet,
                ...
                case 6 when (userSelection == 6):
                    Console.WriteLine("Realizaremos la transferencia entre planetas.\n");
                    foreach (Properties p in planetsListing)
                    {
                        Console.Write(p.id + ".- " + p.name + " Periodo de rotación: " + p.rotation_period + " Periodo orbital: " + p.orbital_period);
                        Console.Write(" Diámetro: " + p.diameter + " Clima: " + p.climate + " Gravedad: " + p.gravity + " Terreno: " + p.terrain);
                    ...
                    listPlanetsNew.Insert(0, new Properties()
                    {
                        ...
                        rotation_period = rotacionTransferencia,
                        orbital_period = orbitaTransferencia,
                        diameter = diametroTransferencia,
